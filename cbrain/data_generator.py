...
class DataGenerator(tf.keras.utils.Sequence):
    ...
    def __init__(self, data_fn, input_vars, output_vars,
        ...
        ### GB is hacking around
        elif type(input_transform) is dict:
            print('Use Input Dictionary Normalizer')
            self.input_transform = DictNormalizer(self.norm_ds, input_vars, input_transform)
        ###
       ...
       ####
        ...
        elif type(output_transform) is tuple:
            print('User Output Transform with Tuple')
            self.output_transform = InputNormalizer(
                self.norm_ds, output_vars, output_transform[0], output_transform[1], var_cut_off)
